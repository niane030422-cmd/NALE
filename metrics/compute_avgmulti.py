...
def get_recall_dict(obj: Dict[str, Any]) -> Optional[Dict[str, float]]:
    """
    读取每个文件里的 "Character 3-gram Recall" 字段（8个语言）
    """
...
def load_one_file(fp: str):
    """
    返回:
      q_lang: query language (obj["language"])
      recall_dict: {target_lang: recall_score}
    """
    try:
...
#    model_list = ("Qwen3-8B", "Qwen3-30B-A3B-Instruct-2507")
    model_list = ( 'gpt-5.1' ,  )
    ...
    for model in model_list:
        # scores[q_lang][t_lang] = [v1, v2, v3 ...] (多个seed文件的值)
        scores = defaultdict(lambda: defaultdict(list))
        missing_files = []

        # 搜索该模型 + retriever 下所有 result.json
        ...
        # summary: 每个 query language 的平均（跨8列）
        for q_lang in LANGUAGES:
