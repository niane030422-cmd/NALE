...
def main():
    ...
    for inst_idx, ins in enumerate(instances):
        ...
        for q_lang in LANGS:
            ...
            # Build a GLOBAL POOL: chunks from all doc languages, with lang tag
            ...
            # ---- A) language distribution in topK
            ...
            # ---- B) answer hit type probing in topK
            hit_counter = Counter()
            for i in top_ids:
                d_lang = pool_langs[i]
                ch = pool_chunks[i]
    ...
    # -------------------------
    # Heatmap matrix: avg share of doc_lang in topK for each q_lang
    heat_data = []
    for q in LANGS:
    ...
    # -------------------------
    # Plot 1: Heatmap (matplotlib imshow)
    # -------------------------
    plt.figure()
