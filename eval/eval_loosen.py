...
def checkanswer(prediction, lang_gt, lang):
    ...
    for k, lang_gt in  lang_gt.items():
        ...
        if len(code_switch_ngram) > 0:
            for i_lang in range(len(ngram_label)):
                code_switch_ngram[i_lang] = max(code_switch_ngram[i_lang], tmp_ngram_label[i_lang])
        else:
...
if __name__ == '__main__':
    ...
    if not os.path.exists(f'./results/'):
        os.mkdir(f'./results/')
    if not os.path.exists(f'./results/cross_lingual/'):
        os.mkdir(f'./results/cross_lingual/')
    if not os.path.exists(f'./results/cross_lingual/{args.dataset}_{args.description}/'):
        os.mkdir(f'./results/cross_lingual/{args.dataset}_{args.description}/')
    ...
    prompt = yaml.load(open('./instructions/instruction_cross.yaml', 'r'), Loader=yaml.FullLoader)
