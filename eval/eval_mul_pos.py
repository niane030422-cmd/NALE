...
def processdata(instance, noise_rate, chunk_num, filename, language, mode="golden", noise_doc='', contriever=None, con_tok=None, stanza_pipeline=None, tokenizer=None, languages=None):
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        choices=['golden','relevant','noise','hybird', "same_topic", "same_topic_hybird", "conflict", "conflict_diff", "cl_ans_text", 'pos']
    ...
    filename = f'{resultpath}/{modelname}/prediction_n_pos_{args.pos}_pl{args.pos_lang}_{args.dataset}_ql{args.language}_multi_conflict_{modelname}_noise{args.noise_rate}_chunk{args.chunk_num}.json'
    ...
    with open(filename, mode, encoding='utf8') as f, open(ans_file) as f2:
        ...
        for instance, ans_item in tqdm.tqdm(zip(instances, ans_data)):
            ...
            if args.chunk_num == 0:
                ...
                query, ans, lang_docs = processdata(instance, args.noise_rate, args.chunk_num, args.dataset, args.language, args.mode, noise_doc=noise_doc, contriever=contriever, con_tok=con_tok, stanza_pipeline=stanza_lang_dict, tokenizer=model_tokenizer, languages=languages)

            try:

                accept_sw_labels, accept_em_labels, ngram_all, prediction, factlabel, docs = predict(query, ans, lang_docs, model, system, instruction, args.dataset, args.language, stanza_list=stanza_lang_dict, languages=languages, pos=args.pos, pos_lang=args.pos_lang)
    ...
    json.dump(scores,open(f'{resultpath}/{args.modelname}/prediction_pos_{args.pos}_pl{args.pos_lang}_{args.dataset}_ql{args.language}_lm{args.language_mode}_{args.modelname}_noise{args.noise_rate}_chunk{args.chunk_num}_seed{args.seed}_result.json','w'),ensure_ascii=False,indent=4)
