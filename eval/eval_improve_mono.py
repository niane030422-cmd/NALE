...
# from llm.gpt import GPT # 如果需要请取消注释
...
def processdata(instance):
    """
    直接从输入实例中提取 query, ans 和 docs (已经是 chunks)
    """
    query = instance.get("query", "")
    ans = instance.get("ans", "")
    
    # 如果答案是以逗号分隔的字符串，转为列表进行多答案匹配
...
if __name__ == '__main__':
    ...
    # 虽然不再真正做检索，但保留这个参数用于输出路径命名以兼容旧格式
    parser.add_argument('--retriever_name', type=str, default='pre_chunked')
#CUDA_VISIBLE_DEVICES=1 python ./eval/eval_improve_mono.py --dataset ./data/rag_eval/translated_Arabic_Chinese.json --language zh
#CUDA_VISIBLE_DEVICES=1 python ./eval/eval_improve_mono.py --dataset ./data/rag_eval/translated_Arabic_English.json --language en
    ...
    for res in results:
        ...
        if labels == [-1]:
            refuse_count += 1
            # 拒绝回答通常在召回率计算中计为 0，或者根据需求剔除。这里计为 0
            n_gram_sum += 0
            valid_count += 1
        else:
            n_gram_sum += sum(labels) / len(labels) # 取多个标准答案 recall 的平均
            valid_count += 1
